...
```javascript
var addTwoNumbers = function(l1, l2, carry = 0) {  // JS 支持默认参数，与 Python carry=0 等价
...
var addTwoNumbers = function(l1, l2) {
    ...
    // 给短的链表前面（对个位来说"前面"即高位）补零
    const padZeros = (head, count) => {
        const dummyShort = new ListNode(0);
        let cur = dummyShort;
