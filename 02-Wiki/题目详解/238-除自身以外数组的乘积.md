...
> **专题归类：** [[02-Wiki/专题总结/数组]] · [[02-Wiki/专题总结/前缀和]]
...
```javascript
var productExceptSelfExtraSpace = function(nums) {
    const n = nums.length;
...
```javascript
var productExceptSelfDiv = function(nums) {
    let total = 1;
    ...
    for (const num of nums) {
        if (zeroCount === 1) {
            res.push(num === 0 ? total : 0);
        } else {
            res.push(Math.trunc(total / num));  // 对应 Python 的整除 //
        }
    }
    return res;
};
...
```javascript
var arrayExceptSelf = function(nums) {
    const n = nums.length;
    const res = new Array(n).fill(1);  // 初始值：乘法单位元
