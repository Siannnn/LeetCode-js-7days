...
| 项目 | 复杂度 | 说明 |
|------|--------|------|
| 时间 | **O(\|s\| + \|t\|)** | 统计 need 需 O(\|t\|)；right 与 left 各自单调移动，全程各最多走 \|s\| 步，窗口判断由 `valid` 变量保证 O(1) |
| 空间 | **O(\|Σ\|)** ≈ O(1) | `need` + `window` 哈希表，最多存 52 个大小写字母（思路二的数组版固定 128 长度，同样是 O(1)） |

> **关键疑点：** for 里面套 while 不是 O(n²)！`left` 从不回退，整个算法生命周期内最多移动 n 次，left 的移动成本均摊到所有轮次后总和仍是线性的（**均摊分析**）。若没有 `valid` 变量、每次遍历 `need` 检查覆盖，会退化为 O(n × \|Σ\|)——这是模板精髓所在。
...
var minWindowArray = function(s, t) {
    ...
    const needKinds = need.filter(v => v > 0).length;
    let valid = 0, left = 0;
    ...
    for (let right = 0; right < s.length; right++) {
        ...
        while (valid === needKinds) {
            ...
            const leftIdx = s.charCodeAt(left);
            if (need[leftIdx] > 0) {
                if (window[leftIdx] === need[leftIdx]) valid--;
                window[leftIdx]--;
            }
            left++;
        }
...
| 项目 | 复杂度 | 说明 |
|------|--------|------|
| 时间 | **O(\|s\| + \|t\|)** | 与思路一相同；`charCodeAt` 取索引 + 数组计数比 Map 增删更快（常数更小），但量级不变 |
| 空间 | **O(\|Σ\|)** = O(128) ≈ **O(1)** | 两个固定长度 128 的计数数组 |
...
```javascript
var slidingWindowComplex = function(s, target) {
    // ===== 1. 初始化 =====
    const need = new Map();       // 目标需求
    for (const ch of target) need.set(ch, (need.get(ch) || 0) + 1);

    const window = new Map();     // 窗口计数
    let valid = 0;                // 已满足的「条件」数量
    let left = 0;

    // 结果记录（找最短用 Infinity，找最长用 0）
    let resultStart = 0;
    ...
    for (let right = 0; right < s.length; right++) {
        ...
        if (need.has(val)) {
            ...
            if (window.get(val) === need.get(val)) valid++;
        }

        // ----- 2b. 缩小窗口 -----
        while (valid === need.size) {  // 所有条件都满足时缩小
            // 更新最优解（找最短：缩小前更新；找最长：扩大后更新）
