...
var rotate = function(nums, k) {
    ...
    const reverse = (l, r) => {
        while (l < r) {
            [nums[l], nums[r]] = [nums[r], nums[l]];  // 解构赋值交换
            l++;
            r--;
        }
    };
...
```javascript
var rotateCycle = function(nums, k) {
    const n = nums.length;
    k %= n;
    ...
    for (let start = 0; start < n; start++) {
        ...
        while (true) {
            const nxt = (cur + k) % n;
            [nums[nxt], prev] = [prev, nums[nxt]];  // 交换：把 prev 放入，取出被覆盖的值
            cur = nxt;
            count++;
            if (cur === start) break;  // 回到起点，一个环完成
        }
    }
};
```
...
```javascript
var rotateCopy = function(nums, k) {
    const n = nums.length;
    k %= n;
