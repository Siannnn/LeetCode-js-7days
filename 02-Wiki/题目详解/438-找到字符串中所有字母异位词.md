...
var findAnagrams = function(s, p) {
    ...
    // 数组比较等价于 Python 的 s_count == p_count
    const isEqual = () => need.every((v, i) => v === window[i]);

    if (isEqual()) res.push(0);

    // 滑动窗口：右边进一个，左边出一个
    for (let i = pLen; i < sLen; i++) {
        window[s.charCodeAt(i) - a]++;                          // 右边界进入
        window[s.charCodeAt(i - pLen) - a]--;                   // 左边界移出
        if (isEqual()) res.push(i - pLen + 1);
    }
...
| 项目  | 复杂度                                           | 说明                                                                   |
| --- | --------------------------------------------- | -------------------------------------------------------------------- |
| 时间  | **O(n × Σ)** = O(26n) ≈ **O(n)**，n = `len(s)` | 初始化统计 p 需 O(m)；随后每个位置 O(1) 滑动更新 + 一次 O(Σ) = O(26) 的数组比较（Σ 为字符集大小 26） |
| 空间  | **O(Σ)** = O(26) ≈ **O(1)**                   | 两个长度 26 的计数数组，与 n、m 无关；输出列表不计入                                       |

> **注意：** 如果字符集不是固定 26 个小写字母（例如 Unicode），则退化为 O(n × Σ)，此时应改用思路二的 diff 优化。
...
var findAnagrams = function(s, p) {
    ...
    let differ = count.filter(v => v !== 0).length;
    if (differ === 0) res.push(0);

    for (let i = pLen; i < sLen; i++) {
        // 右边界进入
        let idx = s.charCodeAt(i) - a;
        if (count[idx] === 0) differ++;
        count[idx]--;
        if (count[idx] === 0) differ--;

        // 左边界移出
        idx = s.charCodeAt(i - pLen) - a;
        if (count[idx] === 0) differ++;
        count[idx]++;
        if (count[idx] === 0) differ--;

        if (differ === 0) res.push(i - pLen + 1);
    }
 return res;
};
```
...
| 项目  | 复杂度                         | 说明                                                                   |
| --- | --------------------------- | -------------------------------------------------------------------- |
| 时间  | **O(n)**，n = `len(s)`       | 初始化 O(m)；之后每步仅右进 / 左出各一次计数更新 + 常数次 `diff/differ` 增减，均为 O(1)，无需逐次比较数组 |
| 空间  | **O(Σ)** = O(26) ≈ **O(1)** | 一个长度 26 的差值计数数组                                                      |
