...
var reverseList = function(head) {
    // 迭代法：三指针翻转
    let pre = null;   // 已反转部分的头
    let cur = head;   // 当前待反转节点
    while (cur) {
        const nxt = cur.next;  // 先保存下一个节点，防断链
        cur.next = pre;        // 翻转：当前节点指向前一个
        pre = cur;             // pre 前移
        cur = nxt;             // cur 前移
    }
...
var reverseList = function(head) {
    ...
    const newHead = reverseList(head.next);  // 递归反转后续链表
    head.next.next = head;                   // 将当前节点接到尾部
    head.next = null;                        // 断开原 next 指针
    return newHead;
};
```
...
```javascript
let pre = null, cur = head;   // 分别赋值；不能写 let pre, cur = null, head（那会漏掉 pre 的赋值）
while (cur) {
    const nxt = cur.next;
    cur.next = pre;
    pre = cur;
