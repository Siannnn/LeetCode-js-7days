...
> **专题归类：** [[02-Wiki/专题总结/数组]] · [[02-Wiki/专题总结/哈希表]]
...
var firstMissingPositive = function(nums) {
    ...
    for (let i = 0; i < n; i++) {
        // while 循环：不断交换直到当前位置的值归位或无法归位
        // Python 链式比较 1 <= nums[i] <= n 在 JS 中要拆成 &&
        while (nums[i] >= 1 && nums[i] <= n && nums[nums[i] - 1] !== nums[i]) {
            // 关键：先把目标下标存起来，再交换
            // 否则交换过程中 nums[i] 变化，目标位置会跟着错
...
```javascript
var firstMissingPositiveMark = function(nums) {
    const n = nums.length;
...
```javascript
var firstMissingPositiveSort = function(nums) {
    // ⚠️ JS 的 sort 默认按字符串排序，数字必须传比较器！
    // 不传 (a, b) => a - b 的话 [10, 2] 会排成 [10, 2]
    nums.sort((a, b) => a - b);
    let expected = 1;
...
```javascript
var inPlaceHashTemplate = function(nums) {
    const n = nums.length;
...
// 条件判断通用形式：当前位置的值需要归位
const needSwap = (nums, i) =>
    nums[i] >= 1 && nums[i] <= nums.length && nums[nums[i] - 1] !== nums[i];
```
