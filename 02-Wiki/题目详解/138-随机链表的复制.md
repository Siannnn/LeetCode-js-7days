...
var copyRandomList = function(head) {
    ...
    const mapping = new Map();   // 原节点 -> 克隆节点

    // 第一遍：创建所有克隆节点
    // 注意：Map 的键是节点对象，按引用判等，正好建立一一对应
    let cur = head;
    while (cur) {
        mapping.set(cur, new Node(cur.val));
        cur = cur.next;
    }

    // 第二遍：设置 next 和 random
    // Python 的 dict.get 查不到返回 None；JS 的 Map.get 返回 undefined，
    // 用 ?? null 归一，避免链表末尾的 next 变成 undefined 而非 null
    cur = head;
    while (cur) {
        mapping.get(cur).next = mapping.get(cur.next) ?? null;
        mapping.get(cur).random = mapping.get(cur.random) ?? null;
        cur = cur.next;
    }
...
```javascript
var copyRandomList = function(head) {
    // 递归 + 哈希表：闭包直接读写外层变量，无需 Python 的 nonlocal
    ...
    const dfs = (node) => {
        ...
        // 创建克隆节点（先存映射再递归，防止 random 成环时无限递归）
        const clone = new Node(node.val);
        mapping.set(node, clone);
