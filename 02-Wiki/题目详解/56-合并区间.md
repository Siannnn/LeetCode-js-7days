...
> **专题归类：** [[02-Wiki/专题总结/数组]] · [[02-Wiki/专题总结/排序]]
...
```javascript
var mergeTwoPointer = function(intervals) {
...
```javascript
var mergeDiff = function(intervals) {
    // 差分数组思想：将区间端点映射为事件
    const events = [];
    for (const [start, end] of intervals) {
        events.push([start, 0]);    // 0 表示起点
        events.push([end, 1]);      // 1 表示终点
    }
