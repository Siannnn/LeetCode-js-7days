...
var reverseKGroup = function(head, k) {
    ...
    if (count < k) return head;  // 不足 K 个，原样返回
...
var reverseKGroup = function(head, k) {
    ...
    while (true) {
        ...
        // 弹出实现翻转
        while (stack.length) {   // 对应 Python 的 while stack:
            cur.next = stack.pop();
            cur = cur.next;
        }

        head = temp;       // 指向下一组的头
        cur.next = null;   // 断开，防止成环
    }
};
```
...
while (true) {
    ...
    // 2. 翻转当前组
    const groupHead = pre.next;
    const nextGroup = cur.next;
    pre.next = reverse(groupHead, k);  // 前驱指向新头
    groupHead.next = nextGroup;        // 组尾指向下一组
